...
fun interp (env, AST_ID i)          = interp(lookup env i) 
  | interp (env, AST_NUM n)         = RES_NUM(n)
  | interp (env, AST_BOOL b)        = RES_BOOL(b)
  | interp (env, AST_FUN (i,e))     = RES_CLOSURE(RES_FUN(i, e), env)
  | interp (env, AST_APP (e1,e2))   = ( case interp(env, e1) of 
                                        ...
                                        | RES_CLOSURE(RES_FUN(i, e), env1) => 
                                        let val value = interp(env, e2) 
                                          in 
                                            interp(Env(update env1 i value), e)
                                        end
                                      )
...
(*val testValue = AST_APP(AST_FUN("x", 
  AST_APP(AST_FUN("f", AST_APP(AST_FUN("x", AST_APP("f", AST_NUM(3))))), AST_NUM(1))), AST_APP(AST_FUN("y", "x"), AST_NUM(1)))
*)